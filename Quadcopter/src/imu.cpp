...
void* imuHandler(void* arg)
 {
	...
	else
	{
        ...
        /*if (receivedCommands.altitude<1 && MS5611Alt<1)//1m du sol???
        {
            altitude = (0,2*ultrasonicAltitude + 0.3*MS5611Alt +0.5*receivedCommands.altitude)/3;//filtre à complémentarité
             if (MS5611Alt>0.05 && MS5611Alt<0.5)// de 10 à 50 cm du sol ???
             {
                 altitude = (0,6*ultrasonicAltitude + 0.4*MS5611Alt)/2; // ou 0.4*receivedCommands.altitude : le capteur le  lus rapide et précis dans ces conditions
             }
        }

        else {altitude = (0.5*MS5611Alt + 0.5*receivedCommands.altitude)/2;}
*/
 ...
 void makeHome(int MS5611Alt)
 {
     ...
     homeRawPosition.altitude = (GROUND_MS5611_TRUST*MS5611Alt + GROUND_GPS_TRUST*receivedCommands.altitude) / 100;
